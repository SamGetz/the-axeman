extends SceneTree
## DEV TOOL. Loads the imported log_a scene and reports mesh stats so we can
## judge scale (cm vs m), orientation and poly budget.
## Run: godot --headless -s res://core/tools/inspect_log.gd
...
func _init() -> void:
	var scene: PackedScene = load("res://assets/models/axe_basic/axe_basic.fbx")
	if scene == null:
		push_error("inspect_log: could not load axe_basic.fbx")
		quit()
		return
	var root := scene.instantiate()
	print("=== log_a.fbx scene tree ===")
	_walk(root, 0)
	quit()
