...
func _test_shop_identity_copy() -> void:
	...
	_check(shelf_copy.contains("Tempered Woodsman’s Axe")
		and shelf_copy.contains("independent 3%")
		and shelf_copy.contains("current gear + skill")
		and shelf_copy.contains("eligible event 10")
		and not shelf_copy.to_lower().contains("rank"),
		"an axe shelf card shows independent/combined/fairness copy without rank language")
